...
def google_search(question):
    first_page = googlesearch.search(question, num_results=5)
    top_three_result = []
    i = 0
    while i < 5:
...
def answer_question(question):
    try:
        ...
        print(f"Exception at first run: {e}")
        try:
            q_type = classify_question(question)
            if q_type == 'Definition' or q_type == 'Location':
                print("except-wiki")
...
def main():
    ...
    if answer == "Oops! I don't know. Try something else":
        # Provide an image link when the chatbot doesn't know the answer
        image_link = "https://example.com/dont-know-image.jpg"  # replace with an actual image link
        print(image_link)
    else:
        print(answer)
